...
BACKEND_TARGET=/srv/netops/netops-littleProgram/backend/app/routes/netops2026.py
VENV=/srv/netops/netops-littleProgram/backend/.venv
...
for path in "$BACKEND_SOURCE" "$BACKEND_TARGET" "$VENV/bin/python" "$SCRIPT_DIR/install-zhiwei-root-entry.sh" \
  "$CACHE_WARMER_SOURCE" "$SCRIPT_DIR/netops-quality-cache.service" "$SCRIPT_DIR/netops-quality-cache.timer"; do
...
rollback() {
  ...
  systemctl show -p MainPID --value netops-platform-api.service | xargs -r kill -HUP || true
  exit "$rc"
}
trap rollback ERR
...
systemctl show -p MainPID --value netops-platform-api.service | xargs -r kill -HUP
sleep 2
...
install -o root -g root -m 0644 "$SCRIPT_DIR/netops-quality-cache.service" /etc/systemd/system/netops-quality-cache.service
install -o root -g root -m 0644 "$SCRIPT_DIR/netops-quality-cache.timer" /etc/systemd/system/netops-quality-cache.timer
systemctl daemon-reload
systemctl enable --now netops-quality-cache.timer

API_CODE=$(curl --noproxy '*' -k -sS -o /dev/null -w '%{http_code}' -H 'Host: anbo.njcatv.net' \
  https://127.0.0.1:5772/api/netops2026/auth/me)
[[ "$API_CODE" == "401" ]]
