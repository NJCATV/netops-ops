...
| --- | --- | --- | --- | --- |
| 网管前端 | `netops-portal-web` | `481bb8e` 为已部署静态文件基线 | 233 `/srv/netops/netops-portal-web`（源码仍为 `13ae970`） | 本地旧工作区有用户未提交前端修改；暂停自动同步，避免覆盖 |
| 平台 API | `netops-platform-api` | `b792f2f` | 233 `/srv/netops/netops-platform-api` | 已部署为 `netops-platform-api.service`，目录干净 |
| 小程序 | `netops-littleProgram` | `85ec6a6` | 233 `/srv/netops/netops-littleProgram` | 已纳入统一目录，目录干净；业务运行按原发布单元保留 |
...
| AIOps | `netops-aiops` | `38b13cb` | 20 `/home/yvesyuan/netops-staging/netops-aiops` | Compose 仍从旧目录 `/opt/jscn-aiops` 运行；旧工作区有未提交内容，未在无发布窗口的情况下强制覆盖 |
...
- 233 的本地旧工作区 `newGoColletor` 同时包含前端与平台 API 的未提交修改；必须按模块拆分、测试并提交后，才能声称 PC、服务器与 GitHub 三端完全一致。
